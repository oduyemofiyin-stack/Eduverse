// ── SCROLL REVEAL (kinda hacky but it works lol) ──
...
// ── 3D CARD TILT (copied this from a codepen, tweaked the values) ──
...
// ── PARALLAX (needs optimization on mobile) ──
export function initParallax() {
  if (typeof window === 'undefined') return;
  const handleScroll = () => {
    const scrollY = window.scrollY;
    // console.log('parallax scroll:', scrollY);
...
// ── MAGNETIC BUTTONS (fun effect, might remove if it lags) ──
...
// ── SMOOTH CURSOR ── TODO: check perf on older devices
...
// ── FLOWING LINES (Air Effect) — inspired by some Dribbble shot ──
...
// ── STARFIELD (Universe Background) — tweaked the twinkle speed ──
export function initStarfield(canvas) {
  ...
  const shootingInterval = setInterval(() => {
    if (Math.random() > 0.7 && running) addShootingStar();
  }, 3000);

  const animate = () => {
    if (!running) { animFrame = requestAnimationFrame(animate); return; }
